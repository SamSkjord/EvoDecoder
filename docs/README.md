...
- Comma.ai’s OpenDBC generator (`tesla_radar_bosch.py`) documents Tesla’s radar CAN layout: it spells out 0x2A9 `GTW_carConfig` bit positions (FourWheelDrive, steering rack mismatch flags, etc.) along with the radar point messages (0x3xx). Handy reference when we reconcile our field guesses with open-source datasets.
