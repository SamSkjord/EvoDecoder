...
class TeslaRadarProtocol:
    ...
    def send_398_message(self):
        ...
        signals = {
            "GTW_performanceConfig": 2,  # Performance spec
            "GTW_fourWheelDrive": 1 if self.force_awd else 0,
            "GTW_unknown1": 0,
            "GTW_dasHw": 2,
            "GTW_parkAssistInstalled": 1,
            "GTW_forwardRadarHw": 1,
            "GTW_airSuspensionInstalled": 1,
            "GTW_unknown2": 0,
            "GTW_country": 826,  # UK (RHD VIN)
            "GTW_parkSensorGeometryType": 1,
            "GTW_rhd": 1,
            "GTW_bodyControlsType": 1,
            "GTW_radarPosition": self.radarPosition,
            "GTW_rearCornerRadarHw": 0,
            "GTW_frontCornerRadarHw": 0,
            "GTW_epasType": self.radarEpasType,
            "GTW_chassisType": 1,
            "GTW_wheelType": 4,
            "GTW_rearSeatControllerMask": 0,
            "GTW_foldingMirrorsInstalled": 1,
            "GTW_euVehicle": 1,
            "GTW_brakeHwType": 2,
            "GTW_autopilot": 3,
            "GTW_unknown3": 0,
...
def _load_plant_failure_map():
    ...
    base_dir = os.path.dirname(__file__)
    radc_path = os.path.join(base_dir, "tesla_odj", "Model S", "RADC.odj.json")
    if not os.path.exists(radc_path):
        _PLANT_FAILURE_MAP = {}
        return _PLANT_FAILURE_MAP

    try:
        with open(radc_path, "r", encoding="utf-8") as fh:
            data = json.load(fh)
        enum_map = (
            data.get("routines", {})
            .get("PLANT_MODE", {})
